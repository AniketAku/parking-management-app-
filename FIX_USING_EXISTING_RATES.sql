...
BEGIN
  RAISE NOTICE '';
  RAISE NOTICE '📊 STEP 1: LOADING RATES FROM YOUR EXISTING SYSTEM...';
  RAISE NOTICE '';
  ...
  IF v_rates IS NOT NULL THEN
    ...
    RAISE NOTICE '✅ Loaded rates from app_settings:';
  ...
  ELSE
    ...
    RAISE NOTICE '⚠️  Settings not found, using fallback rates:';
    RAISE NOTICE '   Trailer: ₹%', v_trailer_rate;
    RAISE NOTICE '   6 Wheeler: ₹%', v_6wheeler_rate;
    RAISE NOTICE '   4 Wheeler: ₹%', v_4wheeler_rate;
    RAISE NOTICE '   2 Wheeler: ₹%', v_2wheeler_rate;
