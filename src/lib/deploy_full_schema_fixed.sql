-- Fixed parking_entries table deployment for Supabase
...
DROP POLICY IF EXISTS "Authenticated users can view parking entries" ON parking_entries;
DROP POLICY IF EXISTS "Users with write permission can insert parking entries" ON parking_entries;
DROP POLICY IF EXISTS "Users with write permission can update parking entries" ON parking_entries;

-- RLS Policies for parking_entries table
CREATE POLICY "Authenticated users can view parking entries" ON parking_entries
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users 
...
CREATE POLICY "Users with write permission can insert parking entries" ON parking_entries
...
CREATE POLICY "Users with write permission can update parking entries" ON parking_entries
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() AND is_approved = true AND role IN ('admin', 'operator')
