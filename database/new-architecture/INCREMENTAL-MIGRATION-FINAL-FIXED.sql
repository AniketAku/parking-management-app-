-- ================================================
-- INCREMENTAL MIGRATION FOR EXISTING DATABASE (FINAL FIXED VERSION)
...
-- FIXES: 
-- - Removed standalone RAISE NOTICE statements
-- - Fixed TABLE syntax to proper CREATE TABLE AS SELECT
-- - Fixed backup table references
-- - Fixed DROP POLICY on non-existent tables by checking table existence first
-- ================================================
...
BEGIN
    ...
    SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_log') INTO has_audit_log;
    ...
    RAISE NOTICE '   Audit Log: %', has_audit_log;
END $$;
...
-- Create backup tables with proper syntax
DO $$
BEGIN
    -- Create backup table for users
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
        DROP TABLE IF EXISTS backup_migration_users;
        CREATE TABLE backup_migration_users AS SELECT * FROM users;
    ELSE
        CREATE TABLE IF NOT EXISTS backup_migration_users (
            id UUID PRIMARY KEY,
            username TEXT,
            email TEXT,
            password TEXT,
    ...
    -- Create backup table for app_settings
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'app_settings') THEN
        DROP TABLE IF EXISTS backup_migration_app_settings;
        CREATE TABLE backup_migration_app_settings AS SELECT * FROM app_settings;
    ELSE
        CREATE TABLE IF NOT EXISTS backup_migration_app_settings (
            id UUID PRIMARY KEY,
            category TEXT,
            key TEXT,
            value JSONB,
    ...
    -- Create backup table for parking_entries
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'parking_entries') THEN
        DROP TABLE IF EXISTS backup_migration_parking_entries;
        CREATE TABLE backup_migration_parking_entries AS SELECT * FROM parking_entries;
    ELSE
        CREATE TABLE IF NOT EXISTS backup_migration_parking_entries (
            id UUID PRIMARY KEY,
            transport_name TEXT,
            vehicle_type TEXT,
            vehicle_number TEXT,
    ...
    -- Create backup table for audit_log
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_log') THEN
        DROP TABLE IF EXISTS backup_migration_audit_log;
        CREATE TABLE backup_migration_audit_log AS SELECT * FROM audit_log;
    ELSE
        CREATE TABLE IF NOT EXISTS backup_migration_audit_log (
            id UUID PRIMARY KEY,
            table_name TEXT,
            record_id UUID,
            action TEXT,
...
BEGIN
    ...
    SELECT COUNT(*) INTO users_count FROM backup_migration_users;
    SELECT COUNT(*) INTO settings_count FROM backup_migration_app_settings;
    SELECT COUNT(*) INTO entries_count FROM backup_migration_parking_entries;
    SELECT COUNT(*) INTO audit_count FROM backup_migration_audit_log;
    
    RAISE NOTICE '   ✅ Users backed up: % records', users_count;
    RAISE NOTICE '   ✅ App settings backed up: % records', settings_count;
    RAISE NOTICE '   ✅ Parking entries backed up: % records', entries_count;
    RAISE NOTICE '   ✅ Audit log backed up: % records', audit_count;
END $$;

-- ================================================
-- PHASE 3: DROP EXISTING COMPLEX STRUCTURE SAFELY
-- ================================================
...
BEGIN
    ...
    -- Drop policies BEFORE dropping tables (check table exists first)
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'users') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'parking_entries') THEN
        ...
        DROP POLICY IF EXISTS "parking_entries_insert_operator" ON parking_entries;
        DROP POLICY IF EXISTS "parking_entries_update_operator" ON parking_entries;
    END IF;
    ...
    -- Drop existing enum types that might conflict
    ...
    RAISE NOTICE '   ✅ Old schema and enum types removed safely';
END $$;
...
BEGIN
    ...
    -- Migrate users from backup (with conditional password handling)
    -- Check if backup table has password column and migrate accordingly
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_users' AND column_name = 'password') THEN
        -- Backup has password column - migrate with passwords
        INSERT INTO users (username, email, password, role, is_active, is_approved, last_login, created_at)
        SELECT 
            COALESCE(username, 'user_' || gen_random_uuid()::text),
            email,
            COALESCE(password, 'password123'),
        ...
        ON CONFLICT (username) DO UPDATE SET
            ...
            updated_at = NOW();
    ELSE
        -- Backup has no password column - migrate without passwords and set default
        INSERT INTO users (username, email, password, role, is_active, is_approved, last_login, created_at)
        SELECT 
            COALESCE(username, 'user_' || gen_random_uuid()::text),
            email,
            'password123',  -- Default password for all migrated users
            COALESCE(role, 'operator'),
            COALESCE(is_active, true),
            COALESCE(is_approved, true),
            last_login,
    ...
    RAISE NOTICE '   ✅ Users migrated: % records', users_migrated;
    
    -- Migrate parking entries from backup with dynamic column detection
    -- Check which optional columns exist in backup table
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_parking_entries' AND column_name = 'status') INTO has_status;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_parking_entries' AND column_name = 'payment_status') INTO has_payment_status;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_parking_entries' AND column_name = 'payment_method') INTO has_payment_method;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_parking_entries' AND column_name = 'notes') INTO has_notes;
    SELECT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'backup_migration_parking_entries' AND column_name = 'created_at') INTO has_created_at;
    
    -- Build dynamic SQL based on available columns (handle NULL values for required fields)
    sql_query := 'INSERT INTO parking_entries (transport_name, vehicle_type, vehicle_number, driver_name, entry_time, exit_time, calculated_fee, actual_fee, status, payment_status, payment_method, notes, created_at) SELECT transport_name, vehicle_type, vehicle_number, COALESCE(driver_name, ''Unknown Driver''), entry_time, exit_time, calculated_fee, actual_fee, ';
    
    -- Add status column handling
    IF has_status THEN
        sql_query := sql_query || 'COALESCE(status, ''Active''), ';
    ELSE
        sql_query := sql_query || '''Active'', ';
    END IF;
    
    -- Add payment_status column handling
    IF has_payment_status THEN
        sql_query := sql_query || 'COALESCE(payment_status, ''Pending''), ';
    ELSE
        sql_query := sql_query || '''Pending'', ';
    END IF;
    
    -- Add payment_method column handling
    IF has_payment_method THEN
        sql_query := sql_query || 'payment_method, ';
    ELSE
        sql_query := sql_query || 'NULL, ';
    END IF;
    
    -- Add notes column handling
    IF has_notes THEN
    ...
    -- Add created_at handling and complete query
    IF has_created_at THEN
        sql_query := sql_query || 'COALESCE(created_at, NOW()) FROM backup_migration_parking_entries';
    ELSE
        sql_query := sql_query || 'NOW() FROM backup_migration_parking_entries';
    END IF;
    ...
    RAISE NOTICE '   ✅ Parking entries migrated: % records', entries_migrated;
END $$;
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM users WHERE role = 'admin') THEN
        INSERT INTO users (username, email, password, role, is_active, is_approved) 
        VALUES ('admin', 'admin@parking.local', 'admin123', 'admin', true, true);
...
BEGIN
    ...
    RAISE NOTICE '🔐 THE "DROP POLICY" ERROR IS NOW RESOLVED!';
    RAISE NOTICE '';
