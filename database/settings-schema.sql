...
-- Multi-level configuration architecture with audit trail
-- ===================================================================
...
CREATE POLICY "Users read non-sensitive app_settings" ON app_settings
  ...
  USING (NOT is_sensitive OR auth.jwt() ->> 'role' = 'admin');
...
-- Location settings: Managers can manage their location settings
CREATE POLICY "Location managers manage location settings" ON location_settings
  ...
  USING (
    EXISTS (
      ...
      AND ul.location_id = location_settings.location_id
      AND ul.role IN ('manager', 'admin')
    )
  );
  ...
  LOOP
    BEGIN
      ...
      ELSIF p_location_id IS NOT NULL THEN
        ...
        DO UPDATE SET value = setting_record.value, updated_at = NOW(), updated_by = auth.uid();
        
      ELSE
        -- Update system settings
        UPDATE app_settings 
        SET value = setting_record.value, updated_at = NOW(), updated_by = auth.uid()
...
COMMENT ON FUNCTION get_setting_value(VARCHAR, UUID, UUID) IS 'Resolves setting value with user > location > system hierarchy';
COMMENT ON FUNCTION bulk_update_settings(JSONB, UUID, UUID) IS 'Safely updates multiple settings in a single transaction';